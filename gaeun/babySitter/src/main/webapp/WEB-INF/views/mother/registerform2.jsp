	...
	$(function() {
		...
		var idIsValid = false;
			...
			$('#chkId').on('click', function(event) {
				$('#chkResult').text(""); // 맨 처음에는 텍스트 없음 
				
				$.ajax({
					url: "/babySitter/account/check-duplicate.action",
					type : "post",
			...
			$('#btnBack').on('click', function(event) {
				history.back(); // location.href='term.action'; 대신에 history 사용 
			});
...
<c:import url="/WEB-INF/views/include/header2.jsp" />
	<div class="container">	
		<div class="row">
			...
			<form:form action="register.action" method="post" modelAttribute="mother">
				...
				<fieldset>
					<legend style=" font-weight: bold"><span class="glyphicon glyphicon-folder-tag">
					</span>&nbsp;기본 정보</legend>
					...
					<div class="form-group">
						...
						<input class="form-control" type="password" name="passwd" placeholder="알파벳, 숫자 조합으로 6~15자리" />
						<form:errors path="passwd" cssClass="error" element="div" />
					</div>

					<div class="form-group">
						<label for="inputName">비밀번호 확인 </label> 
						<form:input class="form-control" path="confirmPassword" type="password" name="confirmPassword" />
				...
				<fieldset>
					...
					<div class="buttons" align="center">
						<input class="btn btn-info" type="button" id="btnBack" value="뒤로 "/>&nbsp;&nbsp;
						<input class="btn btn-info" type="button" id="btnRegister" value="등록"  />
					</div>
				<br/><br/>
